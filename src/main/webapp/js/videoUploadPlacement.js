var adType = {'1':'Rich Media','2':'Display','3':'Mobile Web'},
	  ...
	  adOfType = {'1':adSizeRM,'2':adSizeD,'3':adSizeM},
       ...
       grid.jqGrid(
			{ 	url:'rest/client/videoplacementsTable',
				datatype: "json",
				colNames:['ID', 'Name','Duration', 'Impressions', 'Clicks', 'CTR', 'eCPM', 'Income','Note','File', 'Categories', 'Status', 'Accept Agreament&nbsp;', 'Video Tag', 'VAST Tag'], 
				colModel:[
						...
						{name:'duration',  index:'Duration',  placeholder:'In Seconds', editable: true, editrules: {custom: true, custom_func: myCustomCheck}, editoptions: {size:50, maxlength: 256}, search:false},
						...
						{ name: 'fileToUpload', index: 'File', align: 'left', editable: true, edittype: 'file',  editrules: {custom: true, custom_func: myCustomCheck}, editoptions: { enctype: "multipart/form-data"}, 
						...
						{name:'tag', index:'Video Tag',  editable: true, editrules: { edithidden: true },edittype: "textarea", editoptions: { rows: "4" },  width:180, search:false},
						{name:'vas', index:'VAST Tag',  editable: true, editrules: { edithidden: true },edittype: "textarea", editoptions: { rows: "4" },  width:180, search:false}
	...
	$("#list5").jqGrid('filterToolbar');
	
	//button'sclass=
	$("#t_list5").append("<input type='button' id='add_button' value='New Video' style='height:20px;font-size:12px'/>"); 
	$("#add_button").click(
			function(){
			editRow('new');
			}); 
	$("#t_list5").append("<input type='button' id='del_button' value='Delete' style='height:20px;font-size:12px'/>"); 
	$("#del_button").click(
			function(){
				var row_id_s = jQuery('#list5').jqGrid('getGridParam','selrow');
				//var appSum = jQuery('#listcustom2').jqGrid('getCell',row_id_s,0);
				
				jQuery("#list5").jqGrid('delGridRow', row_id_s, 
					  {recreateForm:true,closeAfterEdit:true, 
		    		   closeOnEscape:true,reloadAfterSubmit:true,
		    		   url: "rest/client/videoFileTable",
		    		   datatype: "json"
		    		  } );
			}); 
	$("#t_list5").append("<input type='button' id='clearFilter_button' value='Clear Filter' style='height:20px;font-size:12px'/>"); 
	$("#clearFilter_button").click(
			function(){
				jQuery("#list5")[0].clearToolbar();
			}); 
	
	function editRow(rowid) {
		if (rowid!='new') {
            var cm =  jQuery("#list5").jqGrid('getColProp','acceptAgreament');
	        cm.editable = false;
	        var cm1 =  jQuery("#list5").jqGrid('getColProp','category');
	        cm1.editable = false;
	        cm1.hidden=true;
	        var cm2 =  jQuery("#list5").jqGrid('getColProp','tag');
	        cm2.editable = true;
	        //var cm3 =  jQuery("#list5").jqGrid('getColProp','status');
	        //cm3.editable = true;
		}
		if (rowid ==='new') {
			var cm =  jQuery("#list5").jqGrid('getColProp','acceptAgreament');
	    ...
	    jQuery("#list5").jqGrid('editGridRow', rowid,
	    		  {recreateForm:true,closeAfterEdit:true, closeAfterAdd:true, 
	    		   ...
	    		   afterSubmit : function(response, postdata)  {
	    			   var fileUploadResp = UploadImage(response, postdata);
	    			   var message = "Record was edited/added sucsefully";
	    			   try {
	    				   var obj = JSON.parse( response.responseText );
		    			   var new_id = obj.id;
		    			   var tag = obj.info;
		    			   //jQuery("#list5")('setRowData', new_id, { Id: new_id, tag: obj.info });
	    		   	...
	    		   	afterComplete: function (response, postdata) {
		    			  var obj = JSON.parse( response.responseText );
		    			  var tag = obj.info;
		    			  var status = obj.placementStatus;
		    			  if(status === 1){
		    				  showDialog(tag);
		    			  }
		    		  }
	...
	$( "#list5" ).navGrid('#gridPager',{edit:false,add:false,del:false,refresh:true,search:false, view:false});
	...
	function ajaxFileUpload(id) 
	{
	    ...
	    (
	        {
	            url: 'rest/client/uploadFile?id='+ id,
	            type: 'POST',
	            secureuri: false,
